...
//
INT_PTR CContrastProperties::OnReceiveMessage(HWND hwnd,
                                           UINT uMsg,
                                           WPARAM wParam,
                                           LPARAM lParam)
{
...
HWND CContrastProperties::CreateSlider(HWND hwndParent)
{
    ...
    HWND hwndSlider = CreateWindow(TRACKBAR_CLASS, TEXT(""), Styles,
                                   62, 35, 39, 100,
    ...
    SendMessage(hwndSlider, TBM_SETRANGE, TRUE, 
                MAKELONG(MinContrastLevel, MaxContrastLevel));
...
void CContrastProperties::OnSliderNotification(WPARAM wParam)
{
    switch(wParam)
    {
        ...
        {
            SetDirty();
            signed char Level = (signed char) 
                    SendMessage(m_hwndSlider, TBM_GETPOS, 0, 0L);
