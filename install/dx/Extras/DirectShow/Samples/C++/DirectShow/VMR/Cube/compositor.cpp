...
const DDPIXELFORMAT g_rgTextMirFormats[] = {
    { sizeof(DDPIXELFORMAT), DDPF_RGB, 0, 32, 0xff0000, 0xff00,  0xff, (DWORD) (0xff<<24ul)},
        ...
        if (m_pDDSTextureMirror) {
            ...
            if (ddsd.dwWidth >= *lpdwWidth && ddsd.dwHeight >= *lpdwHeight) {
                ...
                *lpdwHeight = ddsd.dwHeight;
                ...
                // To prevent a severe performance penalty, set the return
                // value and leave the try section.  This will return S_OK
                // to the method's caller without needing to unwind the stack.
                hr = S_OK;
                __leave;
            }
...
*
\**************************************************************************/
HRESULT CMovie::SetStreamMediaType(DWORD dwStrmID, AM_MEDIA_TYPE *pmt, BOOL fTexture)
{
    if (pmt)
    {
...
*
\**************************************************************************/
bool CMovie::DoesSupportNonPow2CondCap(LPDIRECT3DDEVICE7 pD3DDevice)
{
    ASSERT(pD3DDevice);
...
*
\**************************************************************************/
bool CMovie::DoesSupportAnisoCap(LPDIRECT3DDEVICE7 pD3DDevice)
{
    ASSERT(pD3DDevice);
    D3DDEVICEDESC7 ddDesc;
...
*
\**************************************************************************/
bool CMovie::DoesSupportLinearCap(LPDIRECT3DDEVICE7 pD3DDevice)
{
    ASSERT(pD3DDevice);
    D3DDEVICEDESC7 ddDesc;
