...
//----------------------------------------------------------------------------
HRESULT
CDDrawObject::Initialize(HWND hWndApp, TCHAR *achErrorMsg, UINT uintLen )
{
    HRESULT         hr;
    DDSURFACEDESC2  ddsd;
    ...
    if (FAILED(hr)) {
        if( achErrorMsg )
        {
            lstrcpyn( achErrorMsg, TEXT("This sample cannot continue, because DirectDraw object failed to set exclusive cooperative level.\r\n")\
                TEXT("Application will now exit."), uintLen);
        }
        Terminate();
        return hr;
    }
    ...
    if (FAILED(hr)) {
        if( achErrorMsg )
        {
            lstrcpyn( achErrorMsg, TEXT("This sample cannot continue, because video driver does not support 640x480x32 mode.\r\n")\
                TEXT("You may want to modify this sample or use a different video device. Application will now exit."), uintLen);
        }
        Terminate();
        return hr;
    }
    ...
    if (FAILED(hr)) {
        if( achErrorMsg )
        {
            lstrcpyn( achErrorMsg, TEXT("This sample cannot continue, application failed to allocate the primary surface\r\n")\
                TEXT("whith one back buffer (DDSCAPS_PRIMARYSURFACE | DDSCAPS_FLIP | DDSCAPS_COMPLEX | DDSCAPS_3DDEVICE)\r\n")\
                TEXT("You may want to modify this sample or use a different video device. Application will now exit."), uintLen);
        }
        Terminate();
        return hr;
