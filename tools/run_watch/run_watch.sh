...
get_watch_pid() {
 # PID_RET=`ps -ef | grep ${SHELL_NAME} | grep watch | grep ${APP_MD5} | awk '{print $2}'` #修改前，根据下面修改后的起watch的方式修改获取watch的pid方式

  PID_RET=`ps -ef | grep run_watch.sh | grep watch | grep ${APP_MD5} | awk '{print $2}'`
}
...
start() {
  ...
  get_watch_pid
  if [ -z "${PID_RET}" ];then
    #nohup sh ${SHELL_NAME} watch ${APP_MD5} 2>&1 1>/dev/null & 修改前，用sh bash好像起不了watch，所以换成直接跑本脚本调用的方式
   nohup ./run_watch.sh watch ${APP_MD5} 2>&1 1>/dev/null &
   # ./run_watch.sh watch ${APP_MD5} &
    get_watch_pid
