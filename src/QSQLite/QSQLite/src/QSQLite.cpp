...
QSQLite::QSQLite(const std::string& dbFilePath) :
m_db(nullptr)
{
	QSqlDatabase::drivers();
	m_db = new QSqlDatabase(QSqlDatabase::addDatabase("QSQLITE", "QSQLite"));
...
void QSQLite::open(PragmaFlag pragmaFlag)
{
	if (m_db == nullptr)
	{
		return;
...
QSQLitePrepareStatement QSQLite::preparedCreator(const std::string& sqlString)
{
	QSQLitePrepareStatement prepareQuery(m_db);
	prepareQuery.prepare(sqlString.c_str());
	return prepareQuery;
}

QSQLiteResultSet QSQLite::execute(QSQLitePrepareStatement& spPrepareQuery)
{
	try
	{
		QSQLiteResultSet spResult(spPrepareQuery);
		if (!spPrepareQuery.exec())
		{
			return QSQLiteResultSet();
		}
		return spResult;
	}
	catch (...)
...
void QSQLite::init(const std::string& dbFilePath)
{
	if (m_db == nullptr)
	{
		return;
