...
bool RCWndShadow::Create(HWND hParentWnd, const std::wstring& shadowPath, int left, int top, int right, int bottom)
{
    ::ShowWindow(hParentWnd, SW_SHOW);
    Gdiplus::Bitmap* pBmp = Gdiplus::Bitmap::FromFile(shadowPath.c_str());
    if (pBmp == NULL)
    {
        return false;
    }
    SetShadowBitmap(pBmp);
    SetShadowMargin(left, right, top, bottom, 0);
    bool result = RCWndShadow::Initialize();
    if (result == NULL)
    {
        return false;
    }
    ...
    case WM_SIZE:
        {
            ...
            else
            {
                ...
                {
                    if ((pThis->m_lastWndSize.x > (LONG)width) || (pThis->m_lastWndSize.y > (LONG)height))
                    {
...
void GetMonitorRect(HWND hWnd, RECT* pRectMonitor, RECT* pRectWork)
{
    ...
    if (pRectMonitor != NULL)
    {
        ::CopyRect(pRectMonitor, &(mi.rcMonitor));
    }

    if (pRectWork != NULL)
    {
        ::CopyRect(pRectWork, &(mi.rcWork));
    }
}
...
bool RCWndShadow::CheckFullScreenMode( HWND parentHwnd, HWND hwnd)
{
    ...
    //判断是否全屏模式
    RECT rcScreen;
    GetMonitorRect(parentHwnd, &rcScreen, NULL);
    RECT parentRect;
...
bool IsWin7OrLater()
{
    ...
    return ((ovi.dwMajorVersion >= 6) && (ovi.dwMinorVersion >= 1));
}
