...
{
	m_strategyType = RECONSTRUCTION;
}

bool Reconstruction::buy(const IntDateTime& date, StockInfo& stockInfo)
{
	const std::shared_ptr<ReconstructionInfo>& reconstructionInfo = std::dynamic_pointer_cast<ReconstructionInfo>(m_strategyInfo);

	auto& stockFund = reconstructionInfo->m_fund;
	std::shared_ptr<StockMarket>& spMarket = reconstructionInfo->m_spMarket;
	std::shared_ptr<StockBollIndicator>& spStockBollIndicator = std::dynamic_pointer_cast<StockBollIndicator>(reconstructionInfo->m_spIndicator.find("boll")->second);
...
bool Reconstruction::sell(const IntDateTime& date, StockInfo& stockInfo)
{
	const std::shared_ptr<ReconstructionInfo>& reconstructionInfo = std::dynamic_pointer_cast<ReconstructionInfo>(m_strategyInfo);
