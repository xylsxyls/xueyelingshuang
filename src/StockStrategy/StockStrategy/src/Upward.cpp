...
Upward::Upward()
{
	m_strategyType = UPWARD;
}

bool Upward::buy(const IntDateTime& date, StockInfo& stockInfo)
{
	const std::shared_ptr<UpwardInfo>& upwardInfo = std::dynamic_pointer_cast<UpwardInfo>(m_strategyInfo);

	auto& stockFund = upwardInfo->m_fund;
	std::shared_ptr<StockMarket>& spMarket = upwardInfo->m_spMarket;
	std::shared_ptr<StockBollIndicator>& spStockBollIndicator = std::dynamic_pointer_cast<StockBollIndicator>(upwardInfo->m_spIndicator.find("boll")->second);
...
bool Upward::sell(const IntDateTime& date, StockInfo& stockInfo)
{
	const std::shared_ptr<UpwardInfo>& upwardInfo = std::dynamic_pointer_cast<UpwardInfo>(m_strategyInfo);
