...
void CheckAllStockTask::DoTask()
{
	...
	{
		std::string stock = CStringManager::Mid(allStockTxt.m_vectxt[index][0], 2, 6);
		const std::string& stockClose = allStockTxt.m_vectxt[index][3];
		std::string marketPath = marketFolder + stock + ".txt";
		if (!CSystem::fileExist(marketPath))
		{
			m_stockClient->m_vecErrorStock.push_back(stock);
			continue;
		}
	...
	index = -1;
	while (index++ != m_stockClient->m_vecErrorStock.size() - 2)
	{
		const std::string& stock = m_stockClient->m_vecErrorStock[index];
		allErrorStock.append(stock + ",");
	}
	allErrorStock.append(m_stockClient->m_vecErrorStock[index]);
