...
bool CKeyboard::KeyDown(int code){
	return DmSoft::KeyDown(code) == 1;
}

bool CKeyboard::KeyUp(int code){
	return DmSoft::KeyUp(code) == 1;
}

bool CKeyboard::KeyPress(int code, int sleepTime){
	bool result = true;
	result = result && (DmSoft::KeyDown(code) == 1);
	if (sleepTime == -1) sleepTime = CRandom::Int(KeyPressTimeMin, KeyPressTimeMax);
	Sleep(sleepTime);
	result = result && (DmSoft::KeyUp(code) == 1);
	return result;
}

bool CKeyboard::InputString(const string& str, int sleepTime){
	int times = str.length() * 2 - 1;
	int sleepTimeTemp = sleepTime;
	bool result = true;
	int i = -1;
	while (i++ != str.length() - 1){
		vector<int> vecCode = GetCode(str[i]);
...
bool CKeyboard::KeyManyPress(const vector<int>& vecCode, int sleepTime){
	if (vecCode.size() == 0) return true;
	if (sleepTime == -1) sleepTime = CRandom::Int(KeyPressTimeMin, KeyPressTimeMax);
	vector<int> vecSleepTime = CRandom::Hash(sleepTime, vecCode.size() * 2 - 1);
	bool result = true;
	int i = -1;
...
vector<int> CKeyboard::GetCode(char c){
