"$CLOUD_REBUILD" CStringManager 64 dll release same
"$CLOUD_REBUILD" CSystem 64 dll release same
...
"$CLOUD_REBUILD" Semaphore 64 dll release same
"$CLOUD_REBUILD" CTaskThreadManager 64 dll release only
"$CLOUD_REBUILD" LockFreeQueue 64 dll release same
"$CLOUD_REBUILD" SplitViewerCore 64 dll release only
