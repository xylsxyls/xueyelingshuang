...
void StartTask::DoTask()
{
	...
	while (accountIndex++ != g_config.m_accountCount - 1)
	{
		...
		CMouse::MoveAbsolute(CSystem::getComputerName() == SECOND_COMPUTER ?
			xyls::Point(958, 512) : (CSystem::getComputerName() == FIRST_COMPUTER && g_config.m_screenPixel == 3840 * 2160) ? xyls::Point(2197, 1213) : xyls::Point(1261, 672), 50);
		int32_t count = 3;
		while (count-- != 0)
		{
			CMouse::LeftClick();
	...
	{
		std::string currentExePath = CSystem::GetCurrentExePath();
		ScreenScript::WaitForPic(currentExePath + "res\\titlepic.png", xyls::Rect(0, 0, 50, 50), 0.7, 18000);
	}

	if (CSystem::getComputerName() == SECOND_COMPUTER)
	{
		std::string currentExePath = CSystem::GetCurrentExePath();
		if (!ScreenScript::WaitForPic(currentExePath + "res\\titlepic2.png", xyls::Rect(0, 0, 50, 50), 0.7, 220000))
		{
