...
/*----------------------------创建自己的大数运算库---------------------------------*/
int RSA::cmp(int a1[RSA_MAX], int a2[RSA_MAX])
{
	int l1, l2;
	int i;
	l1 = a1[99];
...
void RSA::mov(int a[RSA_MAX], int *b)
{
	int j;
	for (j = 0; j<RSA_MAX; j++)
		b[j] = a[j];
...
void RSA::mul(int a1[RSA_MAX], int a2[RSA_MAX], int *c)
{
	int i, j;
	int y;
	int x;
...
void RSA::add(int a1[RSA_MAX], int a2[RSA_MAX], int *c)
{
	int i, l1, l2;
...
void RSA::sub(int a1[RSA_MAX], int a2[RSA_MAX], int *c)
{
	int i, l1, l2;
	int len, t1[RSA_MAX], t2[RSA_MAX];
	int k = 0;
	l1 = a1[RSA_MAX - 1];
	l2 = a2[RSA_MAX - 1];
	if ((a1[RSA_MAX - 2] == '-') && (a2[RSA_MAX - 2] == '-'))
...
void RSA::mod(int a[RSA_MAX], int b[RSA_MAX], int  *c)/*/c=a mod b//注意：经检验知道此处A和C的数组都改变了。*/
{
	int d[RSA_MAX] = {};
	mov(a, d);
...
void RSA::divt(int t[RSA_MAX], int b[RSA_MAX], int  *c, int *w)/*//试商法//调用以后w为a mod b, C为a  div b;*/
{
...
void RSA::mulmod(int a[RSA_MAX], int b[RSA_MAX], int n[RSA_MAX], int *m)/*解决 了 m=a*b mod n;*/
{
...
void RSA::expmod(int a[RSA_MAX], int p[RSA_MAX], int n[RSA_MAX], int *m)
{
...
int RSA::is_prime_san(int p[RSA_MAX])
{
...
int RSA::coprime(int e[RSA_MAX], int s[RSA_MAX]) /* 判断两个大数之间是否互质*/
{
...
void RSA::erand(int e[RSA_MAX], int m[RSA_MAX])
{
	int i, k;
	time_t t;
	e[RSA_MAX - 1] = 5;
...
void RSA::rsad(int e[RSA_MAX], int g[RSA_MAX], int *d)
{
...
* @see e，d，n & text
*/
std::string RSA::tencrypto(int e[RSA_MAX], int n[RSA_MAX], const char* text)/*//对有需要的文件进行加密*/
{
