...
BOOL CSerial::Send(char* data){
	BOOL Result = SendData(data,strlen(data));
...
BOOL CSerial::Send(int in,int out){
	//"*1<1!";
	CString strData = "";
	strData.Format("*%d<%d!",out,in);
	int nResult = Send(strData.GetBuffer());
