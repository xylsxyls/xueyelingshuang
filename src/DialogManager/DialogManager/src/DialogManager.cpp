...
void DialogManager::operateDialog(OperateParam& param)
{
	...
	{
		DialogHelper::logFile() << "DESTROY_ALL_OPERATE" << std::endl;
		std::vector<quint64> vecAllDialogId = AllocManager::instance().allDialogId();
		DialogHelper::logFile() << "vecAllDialogId.size = " << vecAllDialogId.size() << std::endl;
	...
	{
		DialogCountOperateParam& dialogCountOperateParam = (DialogCountOperateParam&)param;
		if (dialogCountOperateParam.m_vecOperateType.empty())
		{
			dialogCountOperateParam.m_count = AllocManager::instance().dialogCount();
			break;
		}
		for (auto itType = dialogCountOperateParam.m_vecOperateType.begin(); itType != dialogCountOperateParam.m_vecOperateType.end(); ++itType)
		{
