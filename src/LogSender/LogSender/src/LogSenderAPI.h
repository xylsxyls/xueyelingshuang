...
#define LOG_SEND_LOCAL(format, ...) if (LogSenderManager::instance().getInterface() != nullptr){LogSenderManager::instance().getInterface()->logSend(LogPackage(LogPackage::LOG_INFO, true, true, false, __FILE__, __FUNCTION__), format, ##__VA_ARGS__);}
