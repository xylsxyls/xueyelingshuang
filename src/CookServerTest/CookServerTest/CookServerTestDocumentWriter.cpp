...
std::string CookServerTestDocumentWriter::buildFeatureDocument() const
{
	...
	oss << "\r\n";
	oss << "10. 服务端语音播报。\r\n";
	oss << "POST /api/voice/tts 根据步骤文案和男/女声生成或命中缓存音频，返回 audioUrl 给客户端本地播放。当前支持 windows_sapi、command_wav、command_mp3、command 等提供方，/api/voice/audio?key=... 用于下载缓存音频。\r\n";
	oss << "POST /api/voice/prepare 接收本次做菜流程全部固定语音文本，优先排队生成初始必备语音条；POST /api/voice/fetch 根据 voiceId 长轮询获取已生成音频。服务端通过 CTaskThreadManager 按优先级调度，初始必备语音生成完成后再排队后台语音。\r\n";
	oss << "\r\n";
	oss << "11. 日志和崩溃信息。\r\n";
	...
	oss << "2. 当前账号、购买和收藏状态已经接入 Redis+MySQL 快照存储，MySQL 使用 LONGBLOB 保存快照字节，避免旧本地编码数据写库失败；但还未拆成账号、钱包、视频、点赞、收藏等多张 MySQL 实体表。\r\n";
...
std::string CookServerTestDocumentWriter::buildDeployDocument() const
{
	...
	oss << "3. 当前版本会把日志、dump、语音缓存和视频文件写到 CookServer.exe 同级目录，因此该目录必须允许运行账号读写和创建文件。\r\n";
	oss << "4. 不要把多个同名 CookServer 进程放在同一个目录同时运行，否则日志、缓存、视频文件和端口会互相影响。\r\n";
	oss << "\r\n";
	oss << "三、部署文件\r\n";
	...
	oss << "5. CookServer.ini 是推荐部署的运行配置文件，放在 CookServer.exe 同级目录。需要调整端口、公网 IP、MySQL、Redis、日志、语音合成提供方等配置时，参考 common/CookServer/CookServer.ini，并把该文件复制到 exe 同级目录。\r\n";
	oss << "6. CookServerState.txt 已不再作为运行时存储文件。升级前请先确认旧数据已经迁移到 MySQL，之后部署目录可以不放该文件。\r\n";
	oss << "7. data/voice/cache 是服务端语音缓存目录，由程序运行时自动创建。相同文本、性别、provider 和音频格式会复用缓存，减少重复语音合成和下载流量。\r\n";
	oss << "8. data/videos/files 是第一期视频本地文件目录，由程序运行时按配置使用，后续迁移对象存储时只替换 VideoStorage 实现和 fileUrl 生成逻辑。\r\n";
	oss << "9. 日志文件和 CookServerd_log/CookServer_log 等归档目录由程序运行时自动生成，不需要提前创建。\r\n";
	oss << "\r\n";
	oss << "四、服务器准备\r\n";
	oss << "1. 放通入站 TCP 5207，云服务器还需要在安全组中放通 5207。\r\n";
	oss << "2. 确认同一台机器上没有其他进程占用 5207。\r\n";
	oss << "3. 启动 CookServer 前先启动 MySQL 和 Redis。MySQL 数据库名、账号和密码写在 CookServer.ini；Redis 启动后允许为空，CookServer 会从 MySQL 初始化热数据。\r\n";
	oss << "4. Windows Server 2012 上建议使用专门的低权限运行账号，给部署目录读写权限即可。\r\n";
	oss << "5. 如果要给微信小程序正式环境访问，建议在前面增加 HTTPS 反向代理，由代理监听 443，转发到本机 5207。\r\n";
	oss << "\r\n";
	oss << "五、启动方式\r\n";
	...
	oss << "2. MySQL cook_state_snapshot 表：第一期账号状态最终落库存储，保存 snapshotKey、snapshotValue、updateVersion、updateTime，其中 snapshotValue 是 LONGBLOB 快照字节。\r\n";
	oss << "3. Redis cook:account_store:*：第一期账号状态热层，启动时从 MySQL 初始化，运行时先写 Redis 再后台同步 MySQL。\r\n";
	oss << "4. CookServer.ini：运行配置文件。Voice.TtsProvider 支持 windows_sapi、command_wav、command_mp3、command、disabled。windows_sapi 使用 Windows 本机语音生成 wav；command_wav/command_mp3 会执行 Voice.CommandTemplate，并通过 {textFile}、{audioFile}、{gender}、{format} 占位符交给外部 TTS 程序生成音频。\r\n";
	oss << "5. data/voice/cache：服务端语音缓存目录。/api/voice/tts 负责合成或命中缓存，/api/voice/audio?key=... 负责给客户端下载音频。\r\n";
	oss << "6. data/videos/files：第一期视频本地文件目录，MySQL/Redis 中只保存元数据和路径。\r\n";
	oss << "7. *.log：运行日志，线上排查优先查看最新日志文件和日志归档目录。\r\n";
	oss << "8. *.dmp：程序崩溃时由 CDump 生成的 dump 文件，用于定位崩溃堆栈。\r\n";
	oss << "9. 旧日志归档目录：程序重启后，LogManager 会按当前日志配置归档或清理旧日志。\r\n";
	oss << "\r\n";
	oss << "八、停止、升级和回滚\r\n";
	oss << "1. 停止服务前先确认没有正在执行的测试或客户端请求；正常退出时 CookServer 会等待存储同步线程完成，并把 Redis 最后一版状态补写 MySQL。\r\n";
	oss << "2. 升级前备份整个部署目录和 MySQL cook_server 数据库，至少保留 CookServer.exe、CookServer.ini、res/recipes、日志、dump、data/videos/files；从旧版本升级时，先确认 CookServerState.txt 已迁移到 MySQL。\r\n";
