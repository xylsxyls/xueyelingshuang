...
std::string CookServerTestDocumentWriter::buildFeatureDocument() const
{
	...
	oss << "\r\n";
	oss << "3. 菜谱目录。\r\n";
	oss << "GET /api/recipes 返回分类、菜谱列表和账号信息。当前内置菜谱包括糖醋排骨、电饭锅米饭、番茄炒蛋、蒜蓉青菜、豆腐紫菜汤、黑椒牛柳、照烧鸡腿。前五个为免费/系统菜谱，黑椒牛柳和照烧鸡腿为付费示例菜谱。\r\n";
	oss << "\r\n";
	oss << "4. 推荐流。\r\n";
	oss << "GET /api/feed 返回推荐、精选、关注、好友四个 tab，以及可展示在前端信息流中的菜谱卡片数据，包括标题、描述、作者、价格、点赞数、评论数和关联菜谱 ID。\r\n";
	oss << "\r\n";
	oss << "5. 账号金币。\r\n";
	oss << "GET /api/account 获取账号信息。POST /api/account/recharge 进行金币充值模拟。当前账号状态会保存到 CookServer.exe 同级目录下的 CookServerState.txt。\r\n";
	oss << "\r\n";
	oss << "6. 菜谱购买。\r\n";
	...
	oss << "1. 当前账号、购买和收藏状态使用本地文本文件保存，尚未接入 MySQL/Redis。\r\n";
	oss << "2. 当前支付、提现、评论、关注、好友、发布视频、上传图片、真实内容审核仍是前后端后续阶段功能。\r\n";
	oss << "3. 当前没有真实微信登录鉴权，接口通过 userId 参数模拟不同用户。\r\n";
	oss << "4. 当前 HTTP 服务还不是完整生产网关形态，正式给微信小程序使用时需要 HTTPS、域名备案/配置、反向代理和更完整的限流鉴权。\r\n";
	return oss.str();
}
...
std::string CookServerTestDocumentWriter::buildDeployDocument() const
{
	...
	oss << "3. 当前版本没有独立配置文件，监听端口、监听地址、公网 IP、默认金币、请求体大小等配置仍在程序内部的 Config 中。\r\n";
	oss << "4. CookServerState.txt 是运行时状态文件，不需要首次手动创建。全新部署可以不放该文件；需要保留测试账号金币、购买、收藏状态时，可以随 exe 一起备份和迁移。\r\n";
	oss << "5. 日志文件和 CookServerd_log/CookServer_log 等归档目录由程序运行时自动生成，不需要提前创建。\r\n";
	oss << "\r\n";
	oss << "四、服务器准备\r\n";
	...
	oss << "\r\n";
	oss << "七、运行时文件说明\r\n";
	oss << "1. CookServerState.txt：简化版账号状态文件，保存金币、购买、收藏等状态。\r\n";
	oss << "2. *.log：运行日志，线上排查优先查看最新日志文件和日志归档目录。\r\n";
	oss << "3. *.dmp：程序崩溃时由 CDump 生成的 dump 文件，用于定位崩溃堆栈。\r\n";
	oss << "4. 旧日志归档目录：程序重启后，LogManager 会按当前日志配置归档或清理旧日志。\r\n";
	oss << "\r\n";
	oss << "八、停止、升级和回滚\r\n";
	oss << "1. 停止服务前先确认没有正在执行的测试或客户端请求；当前版本可以直接关闭进程。\r\n";
	oss << "2. 升级前备份整个部署目录，至少保留 CookServer.exe、CookServerState.txt、日志和 dump。\r\n";
	oss << "3. 替换 CookServer.exe 和同批次依赖文件后重新启动服务。\r\n";
	oss << "4. 升级后必须执行 /api/health 验证和 CookServerTest 黑盒测试。\r\n";
	oss << "5. 回滚时停止当前进程，恢复备份文件，再重新启动旧版本。\r\n";
	return oss.str();
}

std::string CookServerTestDocumentWriter::localTextToUtf8(const std::string& text) const
{
#ifdef _WIN32
	if (text.empty())
	{
