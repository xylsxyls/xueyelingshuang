...
std::string ProcessHelper::dataMapName(SharedMemory* position)
{
...
{
	std::string dataMapName = ProcessHelper::dataMapName(position);
...
{
	return new SharedMemory(ProcessHelper::dataMapName(index), ProcessHelper::dataMemoryLength());
}
...
{
	int32_t newPosition = ProcessHelper::dataPosition(data) + length;
	if (newPosition >= ProcessHelper::dataMemoryLength() - (int32_t)(sizeof(int32_t)))
	{
		return false;
	}
	ProcessHelper::dataPosition(data) = newPosition;
	return true;
}
